...
const AddEnquiryModal = ({ visible, onClose, onSubmit, tagOptions = DEFAULT_TAG_OPTIONS }) => {
  ...
  const reset = () => {
    setForm({ ...initialForm, nextFollowUp: todayISO });
    setError('');
  };
  ...
  React.useEffect(() => {
    if (visible) {
      setForm((prev) => ({ ...prev, nextFollowUp: prev.nextFollowUp || todayISO }));
    }
  }, [visible, todayISO]);
